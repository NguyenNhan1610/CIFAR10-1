...
train_step = tf.train.MomentumOptimizer(0.01,0.96).minimize(cross_entropy)
...
target = open("CIFAR10_Norm_wo_Dropout", 'w')
time1 = time.time()
for epoch in range(num_epochs):
  ...
  if epoch %100 ==0:
    ...
    target.write("\n")
    print(line) 
  train_step.run(feed_dict={x: x_batch, y_: y_batch, keep_prob: 1.0})
  if epoch %500 ==0:
